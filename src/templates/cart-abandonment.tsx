...
export interface CartAbandonmentEmailProps extends Partial<Omit<EmailLayoutProps, 'children' | 'theme'>> {
  theme: EmailTheme
  heading?: string
  description?: string
  cartTotal?: string
...
export const CartAbandonmentEmail: React.FC<CartAbandonmentEmailProps> = ({
  ...
  const defaultItems: CartItem[] = [
    { name: 'Limonify UI Team License (Annual)', quantity: 1, price: '$199.00' },
  ]
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <tbody>
            {resolvedItems.map((item, idx) => (
              <tr key={idx}>
                <td style={{ padding: '4px 0', fontSize: '12px', color: theme.foreground }}>
                  {item.name} <span style={{ color: theme.mutedForeground }}>× {item.quantity}</span>
                </td>
                <td style={{ padding: '4px 0', fontSize: '12px', color: theme.foreground, textAlign: 'right', fontWeight: '500' }}>
                  {item.price}
                </td>
              </tr>
            ))}
            <tr style={{ borderTop: `1px solid ${isDark ? '#1f1f1f' : '#f0f0f0'}` }}>
              <td style={{ padding: '10px 0 2px', fontSize: '12px', color: theme.mutedForeground }}>{totalLabel}</td>
              <td style={{ padding: '10px 0 2px', fontSize: '13px', color: theme.foreground, textAlign: 'right', fontWeight: '600' }}>
                {cartTotal}
              </td>
            </tr>
          </tbody>
