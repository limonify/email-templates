...
export interface ProductUpdateEmailProps extends Partial<Omit<EmailLayoutProps, 'children' | 'theme'>> {
  theme: EmailTheme
  version?: string
  heading?: string
  description?: string
...
export const ProductUpdateEmail: React.FC<ProductUpdateEmailProps> = ({
  ...
  const resolvedDescription =
    description || 'Here are the latest features, improvements, and updates shipped in this release:'

  const defaultItems: ChangelogItem[] = [
    {
      title: 'Design System Tokens v2',
      description: 'Synchronized cross-platform color palettes and elevated micro-interactions.',
      tag: 'New',
    },
    {
      title: 'Improved Performance',
      description: 'Reduced bundle sizes by 40% with zero runtime layout shift.',
      tag: 'Improved',
    },
  ]
      ...
      <div style={{ margin: '14px 0 20px', fontFamily: theme.fontFamily }}>
        {resolvedItems.map((item, idx) => (
          <div
            ...
            style={{
              padding: '10px 0',
              borderBottom: idx < resolvedItems.length - 1 ? `1px solid ${itemBorder}` : 'none',
            }}
          >
