...
program
  ...
  .description('Production-grade, design-system-first email templates generator for Go and backend engines')
  .version('1.0.0')
  ...
  .option('--init', 'Generate starter limonify-email.config.json file in current working directory')
  .option('--theme-css <path>', 'Path to your tailwind/CSS stylesheet with :root and .dark variables')
  .option('--mode <mode>', 'Color mode: dark or light', 'dark')
  .option('--engine <engine>', 'Target template engine: go, handlebars, raw')
  .option('--all', 'Generate all templates automatically without interactive prompts')
  ...
  .action(async (options) => {
    ...
    const discoveredConfigPath = options.config || findConfigFile(process.cwd())
    ...
    if (discoveredConfigPath) {
      try {
        ...
        if (loadedConfig.translations) {
          for (const [locale, dict] of Object.entries(loadedConfig.translations)) {
    ...
    const localeChoices = Object.entries(LOCALES_REGISTRY).map(([code, meta]) => ({
      value: code,
      label: `${meta.flag} ${meta.name} (${code})`,
    }))

    const initialLocales: string[] = options.locales || loadedConfig?.locales || ['en', 'tr']
    ...
    if (options.all || options.locales) {
      ...
      const selected = await p.multiselect({
        ...
        initialValues: initialLocales.filter((l: string) => LOCALES_REGISTRY[l]),
        required: true,
      })
    ...
    // 3. Engine selection
    let engine: TemplateEngine = (options.engine as TemplateEngine) || loadedConfig?.engine || 'go'
    if (!options.all && !options.engine && !loadedConfig?.engine) {
      const selectedEngine = await p.select({
        ...
        options: [
          { value: 'go', label: 'Go (html/template)', hint: '{{ .AppName }}, {{ .Code }}' },
          { value: 'handlebars', label: 'Handlebars / Mustache', hint: '{{ appName }}, {{ code }}' },
          { value: 'raw', label: 'Raw Tokens / Unmodified', hint: 'React Email clean compiled HTML' },
        ],
        initialValue: 'go',
      })
    ...
    // 4. Mode Selection
    let mode: 'dark' | 'light' = (options.mode as 'dark' | 'light') || loadedConfig?.mode || 'dark'
    if (!options.all && !options.mode && !loadedConfig?.mode) {
      const selectedMode = await p.select({
        ...
        options: [
          { value: 'dark', label: 'Dark Mode (Deep neutral / #0a0a0a surface)' },
    ...
    // 5. Output directory
    let outputDir = options.output || loadedConfig?.outputDir || './templates/emails'
    ...
    let theme: EmailTheme = mode === 'light' ? { ...defaultLimonifyLightTheme } : { ...defaultLimonifyDarkTheme }
    ...
    if (themeCssPath && fs.existsSync(themeCssPath)) {
      try {
        ...
        p.log.warn(`Could not parse CSS theme from ${themeCssPath}: ${err.message}`)
      }
    }
    ...
    for (const locale of selectedLocales) {
      const localeDir = selectedLocales.length > 1 ? path.join(resolvedOutputDir, locale) : resolvedOutputDir
